...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distXSqrt = (x2 - x1) ** 2;
  const distYSqrt = (y2 - y1) ** 2;

  return Math.sqrt(distXSqrt + distYSqrt);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalarProdOfVectors = x1 * x2 + y1 * y2;
  const vectorModuleX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorModuleY = Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(scalarProdOfVectors / (vectorModuleX * vectorModuleY));
}
...
function roundToPowerOfTen(num, pow) {
  const powersOfTen = `1${''.padStart(pow, '0')}`;

  return Math.round(num / powersOfTen) * powersOfTen;
}
...
function getHypotenuse(a, b) {
  ...
  return Number.isFinite(hypotenuse) ? hypotenuse : Number.MAX_VALUE;
}
...
function getCountOfOddNumbers(number) {
  ...
  const lastOddNum = numberAbs % 2 ? numberAbs : numberAbs - 1;

  return Math.round(lastOddNum + 1) / 2;
}
